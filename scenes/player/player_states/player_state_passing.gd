...
func find_teammate_in_view() -> Player:
	var players_in_view = palyer.teammate_detectio_area.get_overlapping_bodies()
	var teammate_in_view = players_in_view.filter(func (p: Player): return p != palyer)
	teammate_in_view.sort_custom(func (p1: Player, p2: Player):
		var distance1 = p1.position.distance_squared_to(palyer.position)
		var distance2 = p2.position.distance_squared_to(palyer.position)
